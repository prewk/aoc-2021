...
impl From<&str> for Map {
    fn from(input: &str) -> Self {
        ...
        for line in input.lines() {
            ...
            octopi.push(
                line.chars().map(|c| c.to_digit(10).expect("Invalid digit") as u8).collect()
            );
...
impl Map {
    pub fn tick(&self) -> Map {
        // +1 Energy
        let mut wip: Vec<Vec<u8>> = self.octopi
            .iter()
            .map(|row| row
                .iter()
            ...
            for y in 0..self.height {
                for x in 0..self.width {
                    ...
                    if value > 9 && !flashed.contains(&(x, y)) {
                        if y > 0 && x > 0 && !flashed.contains(&(x - 1, y - 1)) { wip[y - 1][x - 1] += 1; }
                        if y > 0 && !flashed.contains(&(x, y - 1)) { wip[y - 1][x] += 1; }
                        if y > 0 && x < self.width - 1 && !flashed.contains(&(x + 1, y - 1)) { wip[y - 1][x + 1] += 1; }

                        if x > 0 && !flashed.contains(&(x - 1, y)) { wip[y][x - 1] += 1; }
                        if x < self.width - 1 && !flashed.contains(&(x + 1, y)) { wip[y][x + 1] += 1; }

                        if x > 0 && y < self.height - 1 && !flashed.contains(&(x - 1, y + 1)) { wip[y + 1][x - 1] += 1; }
                        if y < self.height - 1 && !flashed.contains(&(x, y + 1)) { wip[y + 1][x] += 1; }
                        if x < self.width - 1 && y < self.height - 1 && !flashed.contains(&(x + 1, y + 1)) { wip[y + 1][x + 1] += 1; }
